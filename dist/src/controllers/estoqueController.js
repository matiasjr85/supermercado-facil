...
estoqueController.criarEstoque = async (req, res) => {
    try {
        ...
        res.status(500).json({ mensagem: 'Erro ao criar estoque.', error: error.message });
    }
};
estoqueController.adicionarProduto = async (req, res) => {
    try {
        ...
        const produtoNoEstoque = estoque.produtos.find((item) => item.produto.toString() === produtoId);
        if (produtoNoEstoque) {
        ...
        res.status(500).json({ mensagem: 'Erro ao adicionar produto ao estoque.', error: error.message });
    }
};
...
estoqueController.listarEstoques = async (req, res) => {
    try {
        ...
        res.status(500).json({ mensagem: 'Erro ao listar estoques.', error: error.message });
    }
};
