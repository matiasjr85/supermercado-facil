...
produtoController.obterProdutoPorId = async (req, res) => {
    try {
        ...
        res.status(500).json({ mensagem: 'Erro ao buscar produto por ID.', error: error.message });
    }
};
produtoController.atualizarProduto = async (req, res) => {
    try {
        ...
        const produtoAtualizado = await produtoModel.findByIdAndUpdate(id, dadosAtualizado, { new: true });
