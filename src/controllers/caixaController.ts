...
class caixaController {
    // Criar um novo caixa
    public static criarCaixa = async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            if (id_Funcionario) {
                funcionario = await funcionarioModel.findById(id_Funcionario) as iFuncionario | null;
            if (!funcionario) {
            ...
            res.status(201).json({ mensagem: "Caixa criado com sucesso", caixa: novoCaixa });
        } catch (error) {
    ...
    public static obterCaixas = async (_req: Request, res: Response): Promise<void> => {
        try {
            const caixas = await caixaModel.find().populate("id_Supermercado id_Funcionario id_Cliente");
    ...
    // Obter caixa por ID
    public static obterCaixaPorId = async (req: Request, res: Response): Promise<void> => {
        try {
            const { id } = req.params;
            ...
            const caixa = await caixaModel.findById(id).populate("id_Supermercado id_Funcionario id_Cliente");
    ...
    // Atualizar caixa
    public static atualizarCaixa = async (req: Request, res: Response): Promise<void> => {
        try {
            const { id } = req.params;
            const { id_Supermercado, saldo, id_Funcionario, id_Cliente, status } = req.body;
            ...
            res.status(200).json({ mensagem: "Caixa atualizado com sucesso", caixa: caixaAtualizado });
        } catch (error) {
    ...
    // Excluir caixa
    public static excluirCaixa = async (req: Request, res: Response): Promise<void> => {
        try {
            const { id } = req.params;
