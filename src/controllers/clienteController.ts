...
class clienteController {
  ...
  public static criarCliente = async (req: Request, res: Response): Promise<void> => {
    try {
      
      const { nome, endereco } = req.body as iCliente;

      const novoCliente: iCliente = await clienteModel.create({ nome, endereco });
  ...
  public static obterClientes = async (req: Request, res: Response): Promise<void> => {
    try {
  ...
  public static obterClientePorId = async (req: Request<{ id: string }>, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
  ...
  public static atualizarCliente = async (req: Request<{ id: string }, {}, Partial<iFornecedor>>, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const dadosAtualizados: Partial<iCliente> = req.body;

      const clienteAtualizado: iCliente | null = await clienteModel.findByIdAndUpdate(
        id,
        dadosAtualizados,
        { new: true }
      );
  ...
  public static excluirCliente = async (req: Request<{ id: string }>, res: Response): Promise<void> => {
    try {
      const { id } = req.params;

      const clienteExcluido: iCliente | null = await clienteModel.findByIdAndDelete(id);
